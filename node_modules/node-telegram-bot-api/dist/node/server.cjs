...
exports.startWebhook = startWebhook;
const node_http_1 = __importDefault(require("node:http"));
const node_process_1 = __importDefault(require("node:process"));
const adapters_js_1 = require("../core/adapters.cjs");
/**
 ...
 * ```
 */
function createWebhookServer(bot, options = {}) {
    const path = options.path ?? "/";
    const handler = (0, adapters_js_1.nodeFrameworkWebhook)(bot, options);
 ...
 * ```
 */
async function startWebhook(bot, options) {
    const server = createWebhookServer(bot, options);
    const stop = () => {
        server.close();
    };
    node_process_1.default.on("SIGINT", stop);
    node_process_1.default.on("SIGTERM", stop);
    try {
        ...
        node_process_1.default.off("SIGINT", stop);
        node_process_1.default.off("SIGTERM", stop);
    }
}
