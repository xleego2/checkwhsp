...
const log = (0, debug_js_1.debug)("webhook");
/**
...
function webhookCallback(bot, options = {}) {
    ...
    if (secretToken === undefined) {
        if (allowUnauthenticated !== true) {
            throw new errors_js_1.TelegramBotError("webhookCallback requires `secretToken` (matching setWebhook's secret_token). " +
                "Set it, or pass `allowUnauthenticated: true` if auth is enforced at another layer.", { code: "EPARAM" });
        }
    }
    else if (!SECRET_TOKEN_RE.test(secretToken)) {
        throw new errors_js_1.TelegramBotError("Invalid `secretToken`: must be 1-256 characters of A-Z, a-z, 0-9, _ or - (per setWebhook).", { code: "EPARAM" });
    }
    ...
    return async function handle(request) {
        ...
        if (earlyAck) {
            ...
            const work = Promise.resolve(bot.handleUpdate(update)).catch((err) => {
