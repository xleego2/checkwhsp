...
function serializeParams(params) {
    ...
    for (const [key, value] of Object.entries(params)) {
        ...
        if ((0, files_js_1.isInputFile)(value)) {
            ...
            out[key] = files.length ? (0, files_js_1.formPart)(json, files) : json;
        }
...
function resolve(node, slots, files, depth) {
    if (depth > MAX_DEPTH)
        throw new TypeError("serializeParams: structure too deep (cyclic?)");
    if ((0, files_js_1.isInputFile)(node)) {
        const ref = node.build(slots.next++);
        files.push([ref.slice(files_js_1.ATTACH_PREFIX.length), node]);
        return ref;
    }
