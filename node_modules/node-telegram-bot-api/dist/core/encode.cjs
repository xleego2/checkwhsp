...
const files_js_1 = require("./files.cjs");
const multipart_js_1 = require("./multipart.cjs");
async function encodeForm(fields, 
// Test seam: force the buffered-Blob fallback a streaming runtime never takes.
streaming = (0, multipart_js_1.supportsRequestStreams)()) {
    const strings = [];
    const files = [];
    for (const [key, value] of Object.entries(fields)) {
        if ((0, files_js_1.isInputFile)(value))
            files.push([key, value]);
        else if ((0, files_js_1.isFormPart)(value)) {
    ...
    const { boundary, pieces, replayable } = (0, multipart_js_1.multipartBody)(strings, files);
    const headers = { "content-type": `multipart/form-data; boundary=${boundary}` };
    if (streaming)
        return { headers, body: () => (0, multipart_js_1.streamBody)(pieces), replayable };
    ...
    const blob = await (0, multipart_js_1.bufferBody)(pieces);
