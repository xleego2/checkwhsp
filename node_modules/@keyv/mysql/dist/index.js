...
var KeyvMysql = class extends EventEmitter {
  /**
  ...
  async get(key) {
    ...
    const select = mysql2.format(sql, [key]);
  ...
  async getMany(keys) {
    ...
    const select = mysql2.format(sql, [keys]);
    const rows = await this.query(select);
    const results = [];
  ...
  async set(key, value) {
    ...
    const upsert = mysql2.format(sql, insert);
  ...
  async delete(key) {
    ...
    const del = mysql2.format(delSql, [key]);
    const rows = await this.query(select);
    const row = rows[0];
    if (row === void 0) {
      return false;
  ...
  async deleteMany(key) {
    ...
    const del = mysql2.format(sql, [key]);
  ...
  async *iterator(namespace) {
    ...
    async function* iterate(lastKey, options, query) {
      let sql;
      if (lastKey === null) {
        sql = mysql2.format(
  ...
  async has(key) {
    ...
    const exists = mysql2.format(sql, [key]);
