...
export const makeGroupsSocket = (config) => {
    ...
    return {
        ...
        aiGroupCreate: async (subject, participants = [], options = {}) => {
            ...
            const groupNode = await aiGroupQuery('@g.us', 'set', [{ tag: 'create', attrs: { subject, key }, content: participants.map(jid => ({ tag: 'participant', attrs: { jid } })) }])
            const metadata = extractAIGroupMetadata(groupNode)
            if (autoAddBot !== false && metadata?.id) {
                try {
                    await aiGroupQuery(metadata.id, 'set', [{ tag: 'add', attrs: {}, content: [{ tag: 'participant', attrs: { jid: `${botUser}@bot` } }] }])
                } catch (err) {
