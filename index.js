...
for (let i = 1; i <= 5; i++) {
bot.action(set_acc_, async (ctx) => {
    scanConfig.accountsCount = i;
    await ctx.answerCbQuery(تم اختيار  حسابات);
    await ctx.editMessageText(
...
bot.action(set_delay_, async (ctx) => {
    scanConfig.delayMs = parseInt(d);
    await ctx.answerCbQuery(تم التحديد:  ثانية);
...
bot.action(set_sw_, async (ctx) => {
    scanConfig.switchInterval = sw;
    await ctx.answerCbQuery(التبديل كل  يوزر);
...
try {
    ...
    await startMultiAccountScanner(USERNAMES_FILE, scanConfig, ctx, async (update) => {
        ...
        if (update.type === 'progress' && update.available) {
            await ctx.reply(
                ...
                🔗 **رابط الحجز المباشر:** https://wa.me/qr/\n +
                📊 **الترتيب:** /,
                { parse_mode: 'Markdown' }
            );
        }
    ...
    await ctx.reply(⚠️ **توقف الفحص بسبب خطأ:** );
} finally {
    isRunning = false;
    liveStats.status = 'متوقف ⚪';
}
...
bot.catch((err, ctx) => {
console.error(خطأ في Telegraf ():, err);
});
